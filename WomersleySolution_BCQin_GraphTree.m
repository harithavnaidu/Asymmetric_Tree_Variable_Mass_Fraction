...
function [InpPressureTime,TermPressureTime,qInpTime,qTermTime,...
          ...
          = WomersleySolution_BCQin_GraphTree...
            (T,QnInp,pInpSteady,Table,R,L,h,Ctt,...
...
% Get input pressure in frequency domain for given QnInp
PnInp = zeros(1,NumModes);
PnInp(1) = pInpSteady(1);
for m=2:NumModes
    PnInp(m) = InpImpedance(m,1)*QnInp(m);
end
